    ...
    <!-- Parte central de la pagina --> 
        <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-4">
          <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            <h1 class="h2">Lista de Grupos de Investigacion</h1>
          </div>
...
if (isset($r1)) {
  if (!empty($r1)) {
    $impr = ' <table class="table table-striped table-sm" border=1>
      ...
      <td>Tipo de Unidad de Investigacion</td>
...
foreach ($r1 as $valor) {
  if (isset($valor["nombre"])) {
    ...
    $impr .= '<td>'.$valor['tutor'].'</td>';
    $impr .= '<td>'.$valor['area_de_trabajo'].'</td>';
    $impr .= '<td>'.$valor['unidad_trabajo'].'</td>';
    $impr .= '<td>'.$valor['tipo_unidad_investigacion'].'</td>';
    $impr .= '<td>'.$valor['fecha_registro'].'</td>';

    $impr .= '
    <td>
      <form action="../GrupodeInvestigacion/cargar-grupo_investigacion-controlador.php" method="POST">
      <div class="d-flex justify-content-between align-items-center">
                    <div class="btn-group">
        <input type="hidden" name="nombre" value="'.$valor['nombre'].'" />
        <input type="hidden" name="id_grupo" value="'.$valor['id_grupo'].'" />
        <input class="btn btn-primary" type="submit" value="Actualizar" />
        </div>
        </div>
      </form>
      <form action="../GrupodeInvestigacion/eliminar-grupo_investigacion-controlador.php" method="POST">
         <div class="d-flex justify-content-between align-items-center">
                    <div class="btn-group">
                    <input type="hidden" name="nombre" value="'.$valor['nombre'].'" />
        <input class="d-flex justify-content-between align-items-center" type="hidden" name="id_grupo" value="'.$valor['id_grupo'].'" />
